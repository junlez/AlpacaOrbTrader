...
2. **Entry**: once a subsequent entry candle's *VWAP* (`vw`) crosses above the range high (long) or below the range low (short), it enters a position. The candle resolution is configurable via `--entry-timeframe` (default: 10Min, based on backtesting). The close price (`c`) is also supported via `--entry-field c`.
...
```
python alpaca_orb.py SYMBOL [QTY] [--env-file ENV_FILE] [--entry-timeframe 10Min] [--entry-field vw|c] [--exit-mode prev-hl|close] [--stop-pct N] [--reward-pct N]
```
